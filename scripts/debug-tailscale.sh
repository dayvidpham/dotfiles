...
# Run inside the VM: sudo ./debug-tailscale.sh
...
echo "=== Collecting Tailscale diagnostics ==="
{
    ...
    tailscale debug prefs 2>&1 || echo "tailscale debug prefs failed"

    echo ""
    echo "=== tailscale serve status ==="
    tailscale serve status 2>&1 || echo "tailscale serve status failed"
...
echo "=== Collecting network info ==="
{
    ...
    ss -tlnp 2>/dev/null || netstat -tlnp 2>/dev/null || echo "Cannot list ports"
...
echo "=== Creating summary ==="
{
    ...
    tailscale status --json 2>/dev/null | head -20 || echo "Cannot get tailscale status"
...
echo "To copy out: Copy $TARBALL to host"
